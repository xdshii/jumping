    ...
    def forward(
        ...
        for i, model_type in enumerate(self.model_types):
            ...
            try:
                ...
                # 加权累加到总损失
                total_loss += self.model_weights[i] * identity_loss
                valid_models += 1
        ...
        avg_distance = sum([losses.get(f"{m}_distance", 0) for m in self.model_types if f"{m}_distance" in losses])
        if len([m for m in self.model_types if f"{m}_distance" in losses]) > 0:
            avg_distance = avg_distance / len([m for m in self.model_types if f"{m}_distance" in losses])
