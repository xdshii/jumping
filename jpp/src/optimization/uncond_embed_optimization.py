...
class UnconditionalEmbeddingOptimizer:
    ...
    def compute_reconstruction_loss(
        ...
        # 检查可用内存，如果不足则启用简化模式
        available_memory = self._get_gpu_memory_gb()
        if available_memory < 12.0:  # 适应512×512分辨率需求
            logger.warning(f"GPU内存不足({available_memory:.1f}GB)，启用简化重建损失计算")
            return self._compute_simplified_loss(target_latents, uncond_embeddings, prompt_embeddings, timesteps)
        
        total_loss = 0.0
        num_steps = len(timesteps)
        
        # 内存优化：分批处理时间步而不是一次性处理所有
        batch_size = min(3, num_steps)  # 每批最多3个时间步
        ...
        for batch_start in range(0, num_steps, batch_size):
            ...
            batch_timesteps = timesteps[batch_start:batch_end]
            
            batch_loss = 0.0
            
            for i, timestep in enumerate(batch_timesteps):
                try:
                    # 清理之前的计算缓存
                    self._clear_memory()
                    ...
                    # 执行DDIM反向步骤
                    if batch_start + i < num_steps - 1:
                        with torch.no_grad():
                            noise_pred = noise_pred_uncond + self.guidance_scale * (noise_pred_cond - noise_pred_uncond)
                            if batch_start + i + 1 < len(timesteps):
                                next_timestep = timesteps[batch_start + i + 1]
                    ...
                    logger.error("UNet前向传播内存不足，跳过当前时间步")
                    self._clear_memory()
                    continue
            
            total_loss += batch_loss
            self._clear_memory()  # 批次间清理内存
        
        return total_loss / num_steps
    
    def _compute_simplified_loss(
        self,
        target_latents: torch.Tensor,
        uncond_embeddings: torch.Tensor,
        prompt_embeddings: torch.Tensor,
        ...
        """简化的重建损失计算（极低内存模式）"""
        # 只使用前3个时间步进行快速损失计算
        selected_timesteps = timesteps[:3]
        total_loss = 0.0
        ...
        for timestep in selected_timesteps:
            try:
                ...
                if timestep.dim() == 0:
                    timestep = timestep.unsqueeze(0).repeat(target_latents.shape[0]).to(target_latents.device)
                else:
                    timestep = timestep.to(target_latents.device)
                ...
                logger.warning("极简模式下仍然内存不足，跳过时间步")
                self._clear_memory()
                continue
        
        return total_loss / len(selected_timesteps)
