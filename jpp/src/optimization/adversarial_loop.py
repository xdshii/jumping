...
class AdversarialOptimizer:
    ...
    def prepare_image(
        ...
        target_size: Tuple[int, int] = (256, 256)  # 临时降低分辨率确保稳定运行
    ...
    def optimize_uncond_embeddings(
        ...
        Returns:
            优化后的无条件嵌入（如果优化失败则使用默认空字符串嵌入）
        """
        logger.info("开始优化无条件文本嵌入...")
        
        try:
            # 将潜空间解码为图像
            target_image = self.sd_loader.decode_latents(image_latents)
            
            # 优化无条件嵌入
            optimized_embeddings, optimization_info = self.uncond_optimizer.optimize_unconditional_embeddings(
                target_image=target_image,
                prompt=prompt
            )
            
            logger.info("无条件嵌入优化成功")
            return optimized_embeddings
            
        except torch.OutOfMemoryError as e:
            logger.warning(f"无条件嵌入优化因内存不足失败，使用默认空字符串嵌入: {e}")
            # 清理内存
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
                torch.cuda.synchronize()
            # 返回默认空字符串嵌入
            return self.sd_loader.encode_text("")
            
        except Exception as e:
            logger.error(f"无条件嵌入优化失败，使用默认空字符串嵌入: {e}")
            # 清理内存
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
                torch.cuda.synchronize()
    ...
    def compute_total_loss(
        ...
        # 获取强度对应的权重
        weights = self.config.get_strength_config(strength)
        
        # 1. 身份损失（最大化面部特征距离）
        id_loss_dict = self.id_loss(original_image, protected_image)
        if isinstance(id_loss_dict, dict):
            id_loss_total = id_loss_dict.get("total_loss", torch.tensor(0.0, device=self.device, requires_grad=True))
            losses.update({f"id_{k}": v for k, v in id_loss_dict.items()})
        else:
            id_loss_total = id_loss_dict
            losses["id_total_loss"] = id_loss_total
        
        # 2. LPIPS感知损失（最小化感知差异）
        lpips_loss_result = self.lpips_loss(original_image, protected_image)
        if isinstance(lpips_loss_result, dict):
            lpips_loss_total = lpips_loss_result.get("total_loss", torch.tensor(0.0, device=self.device, requires_grad=True))
            losses.update({f"lpips_{k}": v for k, v in lpips_loss_result.items()})
        else:
            lpips_loss_total = lpips_loss_result
            losses["lpips_total_loss"] = lpips_loss_total
        ...
        try:
            ...
            attention_loss_total = torch.tensor(0.0, device=self.device, requires_grad=True)
            losses["attention_total_loss"] = attention_loss_total
        
        # 计算加权总损失
        total_loss = (
            weights['identity'] * id_loss_total +
            weights['lpips'] * lpips_loss_total +
            weights.get('self_attn', 0.1) * attention_loss_total
        )
        
        losses["total_loss"] = total_loss
        losses["weighted_id"] = weights['identity'] * id_loss_total
        losses["weighted_lpips"] = weights['lpips'] * lpips_loss_total
        losses["weighted_attention"] = weights.get('self_attn', 0.1) * attention_loss_total
